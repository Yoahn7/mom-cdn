require([cdnDomain+'/js/zlib/app.js'], function(App) {
    //获取参数（在bus.js的方法openSelUserWin3中传参）
    var multiple = Mom.getUrlParam('multiple')||'false';//是否多选
    var showSearch = Mom.getUrlParam('showSearch')||'true';//是否显示搜索框
    var checkType = Mom.getUrlParam('checkType')||'0';//按什么选择（0角色/1部门/2人员）默认是部门
    var checkChange = Mom.getUrlParam('checkChange')||'true';//选择类型是否可修改（true/false）,默认为true

    try{$("input[name='checkChange']").trigger("change");}catch(e){}
    if(showSearch != 'true'){
        $('#has_searchText').parent().hide();
    }
    if(checkChange == 'true'){
        $("input[name='checkChange']").change(function(){
            var val = Validator.getRadiosValue('checkChange');
            $('.container').css('display','none');
            $('.container').eq(parseInt(val)).css('display','block');
        });
        Validator.setCheckboxValue('checkChange',checkType);
    }else{
        $('.checkChangeBox').css('display','none');
    }
    require(['ztree_my'],function(ZTree){
        ...
        function loadWaitUserData(){
            ...
            var waitApiCfg = $.extend(true,{},{
                url: Const.admin+"/api/sys/SysUser/list?__noauth=true",
