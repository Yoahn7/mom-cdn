    ...
    I18N = {
        ...
        getLang : function() {
            ...
            if (!!I18N._lang) {
                ...
                var res = EUI.queryObj(EUI.getContextPath()+"i18n.do", params);
                if (!!res) {
                    I18N._lang = res;
                    return I18N._lang;
                }
        ...
        loadResource : function(force) {
            ...
            try {//考虑到可能连不上服务器，将异常抓住
                ...
                if (force || !root.i18n_properties) {
                    ...
                    var ajax = EUI
                        .get({
                            url : EUI.formatUrl(EUI.getContextPath() + "i18n.do?action=loadLang"),
                            async : false
...
+function (namespace, name) {
    ...
    function _createActiveXObject(parentDom, width, height, clsid, codebase, onfinish){
        ...
        idoc.open();
        idoc.writeln('<html><body><object id="plugin" codebase="' + EUI.sys.getContextPath() + codebase + '" classid="' + clsid + '"></object></body></html>');
        idoc.close();
    }
        ...
        if (!(httpStatus >= 200 && httpStatus < 300 || httpStatus === 1223)) {
            ...
            if (httpStatus >= 12000 || httpStatus == 0) {
                if(EUI.browser.isApp){
                    EUI.hideWaitDialog();
                    showMobileTipDlg("当前界面已过期，请重新进入。","提示",function(){
        ...
        getFileContent : function(jsuri) {
            ...
            try {
                ...
                if (hp.status < 200 || hp.status >= 400) {
                    EUI.throwError(I18N.getString("xui.util.js.29",
...
+function(namespace, EUI) {
    ...
    /**向map中加入一个key与value的对应，如果value = undefined 则value=null;
     ...
     并返回旧的value*/
    Map.prototype.extractQuotedStr = function(key, value) {
        ...
        if (EUI.isNumber(i)) {
            ...
            if (this.contains(i)) {
                ...
                if (idx == -1) {
                    ...
                    EUI.throwError(I18N
                        .getString("xui.util.js.4", "map中存在，记录关键字的数组中不存在！"));
                }
    ...
    function searchIndex(content, headStart, end, index) {
        ...
        if (firstEnd == -1) {
            ...
            alert(I18N
                .getString("xui.util.js.23", "没有匹配，检查相同段名的嵌套情况!", [ headStart ]));
        }
        return firstEnd;
    }
    ...
    Download.getInstance = function() {
        return EUI.getObjectFromRoot("Download", "__DownloadInstance__", true,
            "xui/util.js");
    }
    ...
    Download.prototype._initDownload = function() {
        this._downloadAttachForm = new HtmlElementFactory(this.wnd, this.doc.body)
            .form("downloadForm" + Math.floor(Math.random() * 9999), null, "post",
                this.url, true);
    }
    ...
    AbstractReqObj.prototype.showError = function(e) {
        if(EUI.browser.isMobile){
            var title = (typeof (EUI.showError) == "function")?"错误提示":"提示";
    ...
    FormatSequence.prototype.reset = function(options){
        ...
        if(!(sequence instanceof TableSequence)) throw new Error('FormatSequence参数非法【不是TableSequence对象】.');
    ...
    ComplexSequence.prototype.reset = function(options){
        ...
        if(!$.isArray(sequences)) throw new Error('ComplexSequence参数非法【不是TableSequence对象数组】.');
        this._format = typeof(format) == 'function' ? format : null;
            ...
            transStr2Function = function(evtstr, option) {
                ...
                try {
                    ...
                    e.message = I18N.getString(
                        "esmain.pagedesigner.js.portalutil.js.wrongis", "错误：")
                        + '<span style="color: red">'
                        + e.message
                        + '</span>\r\n'
                        + '<div style="padding: 0px 30px;color:#666">'
                        + I18N.getString(
                            "esmain.pagedesigner.js.portalutil.js.checkformat",
                            "请检查自定义事件格式，内容如下：")
                        + '\r\n'
    ...
    function getRootDefaultDlg(options){
        ...
        if(!dlg){
            ...
            if(constructor){
                ...
                console.error("不存在对象【" + objectName + "】");
            }
        }else{
            if(rootEUI.isFunction(onfinish)){
                onfinish(dlg);
        ...
        showWaitDialog: function (msg) {
            ...
            EUI.getDefaultWaitDialog(function(dlg) {
                if (rootWindow.__ESEN$WaitDialog$IsVisible__) {
                    ...
                    msg = msg ? msg : I18N.getString("xui.util.js.20","正在装入，请等待…");
        ...
        getHelp: function (help, title, isModal) {
            ...
            return getRootDefaultDlg({
                ...
                onfinish: function(rs) {
                    ...
                    rs.open(isModal);
                    rs.setTitle(title, I18N.getString("XUI.COMMON.HELP","帮助"));
        ...
        showProgressDlg: function (msg, oncancel) {
            return getRootDefaultDlg({
                ...
                onfinish: function(progressDlg) {
                    progressDlg.setMessage(msg ? msg : /*"正在计算，请稍等......"*/I18N.getString("xui.util.js.28","正在计算，请稍等......"));
