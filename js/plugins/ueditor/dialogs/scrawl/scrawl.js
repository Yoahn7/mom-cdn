    ...
    scrawl.prototype = {
        ...
        getCanvasData:function () {
            ...
            if (!!img) {
                ...
                alert('未获取到画布');
                return ""; //add by Qiyh @Date 200-09-15
...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if (!!base64) {
            var options = {
                timeout:100000,
                data: editor.getOpt('scrawlFieldName')+"="+base64,
                onsuccess:function (xhr) {
                    if (!scrawlObj.isCancelScrawl) {
                        var responseObj = eval("(" + xhr.responseText + ")");
                        if (responseObj.state == "SUCCESS") {
                            var imgObj = {},
                                urlPrefix = editor.getOpt('scrawlUrlPrefix')||editor.getOpt('fileUrlPrefix'),
                                url = urlPrefix + responseObj.url;
                            imgObj.src = url;
                            imgObj._src = url;
