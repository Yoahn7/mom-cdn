...
(function () {
    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                //modify by Qiyh @Date:2020-09-14 ||editor.getOpt('fileUrlPrefix')
                prefix = editor.getOpt('imageUrlPrefix')||editor.getOpt('fileUrlPrefix');
            for (i = 0; i < this.imageList.length; i++) {
                data = this.imageList[i];
                list.push({
                    src: prefix + encodeURIComponent(data.url),
                    _src: prefix + encodeURIComponent(data.url),
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
    ...
    OnlineImage.prototype = {
        ...
        pushData: function (list) {
            var i, item, img, icon, _this = this,
                urlPrefix = editor.getOpt('imageManagerUrlPrefix')||editor.getOpt('fileUrlPrefix');
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    item = document.createElement('li');
                    img = document.createElement('img');
                    icon = document.createElement('span');
                var src = urlPrefix + encodeURIComponent(list[i].url);
                    ...
                    img.width = 113;
                    img.setAttribute('src', src += (src.indexOf('?')<0?'?noCache=':'&noCache=')+ (+new Date()).toString(36) ) ;
    ...
    SearchImage.prototype = {
        ...
        getImageData: function(){
            ...
            $G('searchListUl').innerHTML = "<p style='text-align:center'>"+lang.searchLoading+"</p>";
            ajax.request(url, {
                'dataType': 'jsonp',
                'charset': 'GB18030',
