...
//初始化区域图的对象
    ...
    if(property.haveTool){
        ...
        this.$tool.append("<div style='margin-bottom:5px'><span/><span/><span/><span/></div>"
            +"<a href='javascript:void(0)' type='cursor' class='GooFlow_tool_btndown' id='"+this.$id+"_btn_cursor'><i class='ico_cursor'/><em>选择指针</em></a>"
            +"<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><i class='ico_direct'/><em>入口节点</em></a>"
        );
        //lhy
        if(property.toolBtns&&property.toolBtns.length>0){
            tmp="<span/>";
            for(var i=0;i<property.toolBtns.length;++i){
                tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+property.toolBtns[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_"+property.toolBtns[i]+"'/><em></em></a>";//加入自定义按钮
            }
            this.$tool.append(tmp);
        }
        //加入区域划分框工具开关按钮
        if(property.haveGroup)
            this.$tool.append("<span/><a href='javascript:void(0)' type='group' class='GooFlow_tool_btn' id='"+this.$id+"_btn_group'><i class='ico_group'/><em></em></a>");
        toolWidth=94;
        this.$nowType="cursor";
        //绑定各个按钮的点击事件
        this.$tool.on("click",{inthis:this},function(e){
            ...
            switch(e.target.tagName){
                ...
                case "EM":	tar=e.target.parentNode;break;
                case "A":	tar=e.target;
            }
...
GooFlow.prototype={
    ...
    setNodeRemarks:function(remark){
        ...
        this.$tool.children("a").each(function(){
            var titleName = remark[$(this).attr("id").split("btn_")[1]];
            $(this).attr('title',titleName);
            // this.title=titleName;
            $(this).find('em').text(titleName);
        });
...
///////////以下为有关画线的方法
    ...
    addLine:function(id,json){
        ...
        // 流转方向
        if(json.direction ==undefined)	this.$lineData[id].direction='1';
        // 流转条件
        if(json.remark ==undefined)	this.$lineData[id].remark='';
