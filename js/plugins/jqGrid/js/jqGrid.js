	...
	return this.each( function() {
		...
		$(grid.bDiv)
           ...
           /* .css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: (grid.width)+"px"})*/
			// by lhy 将设置的高度值改为最大高度，体验较好
			.css({ maxHeight: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: (grid.width)+"px"})
		...
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
                    ...
                    // by lhy 冻结列时冻结列的高度值与列的高度值应该相等。
					$($t.grid.fbDiv).height($($t.grid.bDiv).height());
					console.log($t.grid.fbDiv);
...
// Form Functions
// 	debugger   BY LHY  为处于编辑状态行中的具有"editable:true"单元格根据eltype类型动态创建该类型的dom然后插入到其父元素td中
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if(!this.edittype) {this.edittype = "text";console.log('text')}
...
//Editing
	editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		return this.each(function(){
			...
			// debugger;  当前行（1）编辑状态，单击另一行（2），行1设为非编辑，行2处于编辑状态
		...
		this.each(function(){
			...
			if( tarspan.hasClass(minus) ) {
				...
				if(r){
					...
					while(r) {
						/*console.log($(r))
                        console.log($(r).hasClass('ui-subgrid'));*/
						// if($(r).hasClass('ui-subgrid')) break;
						itemGroupingLevel = getGroupingLevelFromClass(r.className);
						...
						if (itemGroupingLevel !== undefined) {
							...
							//当分组表头下嵌套主子表结构时，分组表头在操作展开、折叠时不会记录主子表的展开、折叠状态。以下代码就是解决不会记录主子表状态的问题。
							// by lhy S  20200520
							if($(r).hasClass('ui-subgrid')){
								var preSibling = $(r).prev(),
									firstTdClass = $(preSibling).find('td:first').attr('class');
								// 展开
								if(firstTdClass.indexOf('sgexpanded')>-1){
                                    $(r).show();
                                    if(frz) {
                                        $(r2).show();
                                    }
